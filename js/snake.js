  ...
  (function () {
    ...
    */
    return function (config) {
       ...
       */
      me.eatFood = function () {
        var foodGrowthFactor = playingBoard.myFood.getGrowthFactor(); // Get the growth factor from the food

        // Calculate actual growth based on the food type eaten
        var actualGrowth = Math.round(growthIncr * foodGrowthFactor); 

        if (blockPool.length <= actualGrowth) {
          createBlocks(actualGrowth * 2);
        }
        var blocks = blockPool.splice(0, actualGrowth);
...
SNAKE.Food =
  ...
  (function () {
    ...
    function getRandomFoodType() {
      ...
      if (randomNum < 0.33) {
        return { color: "red", growthFactor: 1 }; // Red food increases length normally
      } else if (randomNum < 0.66) {
        return { color: "purple", growthFactor: 1.5 }; // Purple food increases length by 1.5 times
      } else {
        return { color: "blue", growthFactor: 0.75 }; // Blue food increases length by 0.75 times
      }
    }
    ...
    */
    return function (config) {
      ...
      var currentFoodType = getRandomFoodType(); // Get random food type upon creation

      var elmFood = document.createElement("div");
      elmFood.setAttribute("id", "snake-food-" + myId);
      elmFood.className = "snake-food-block " + currentFoodType.color; // Add food color to class for CSS styling
      ...
      me.randomlyPlaceFood = function () {
        ...
        currentFoodType = getRandomFoodType(); // Determine new food type
        elmFood.className = "snake-food-block " + currentFoodType.color;
        ...
        /**
        * Provides the growth factor for the current food type.
        * @method getGrowthFactor
        * @return {Number} The growth factor for the current food type.
        */
        me.getGrowthFactor = function() {
          return currentFoodType.growthFactor;
      };      
    };
  })();
